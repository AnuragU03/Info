...
export function VillageCard({ village, name, location, shortDescription, discoverText, regionText }: VillageCardProps) {
      ...
      <Link href={`/villages/${village.id}`} className="block">
        <div className="relative">
          <Image
            ...
            data-ai-hint={`${regionText.toLowerCase()} landscape`}
          />
          <Badge variant="secondary" className="absolute top-3 right-3">{regionText}</Badge>
        </div>
        <CardContent className="p-4">
          <h3 className="text-xl font-headline font-bold text-foreground mb-1">{name}</h3>
          ...
          <Button variant="ghost" className="w-full justify-start p-0 h-auto text-primary hover:text-primary/80">
            {discoverText} <ArrowRight className="h-4 w-4 ml-2" />
          </Button>
        </CardContent>
      </Link>
    </Card>
